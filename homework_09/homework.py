from pywebio.input import textarea, input as input_pw, NUMBER, DATE, TEXT, select, slider, checkbox, radio
...
def show_thank_you(username: str, rating: int) -> None:
  if rating > 5:
    put_success(
      constants.MESSAGE_GOOD_MOVIE.format(
        username=username
      )
    )
  else:
    put_warning(
      constants.MESSAGE_BAD_MOVIE.format(
        username=username
      )
    )
def main():
  ...
  movie_category = select(constants.REQUEST_MOVIE_CATEGORY, constants.MOVIE_CATEGORIES)
  ...
  movie_emotion = checkbox(constants.REQUEST_USER_EMOCION, constants.MOVIE_EMOCION)
  movie_recommendation = radio(constants.REQUEST_RECOMMENDATION, constants.MOVIE_RECOMMENDATION)
